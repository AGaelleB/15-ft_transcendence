...
export let isTournament3D = false;
...
export async function initializeTournament3D() {
    ...
    document.getElementById('startTournamentButton').addEventListener('click', () => {
        ...
        createTournamentMatches3D(tournamentPlayers);
        startNextMatch3D();
    });
}
...
export function createTournamentMatches3D(playerNames) {
    const matchQueue = [];
...
export function startNextMatch3D() {
    ...
    if (matchQueue.length === 0) {
        if (winners.length === 1) {
            ...
            if (winners[0] === user.username)
                ...
                result = "D";         
            sendGameResult(0, 0, user.id, "3d", "T", result);
    ...
    currentMatchPlayers3D = { player1, player2 };
    localStorage.setItem("tournamentMatches", JSON.stringify(matchQueue));

    // Redirect "Mr Robot" 
    if (player2 === "Mr Robot" || player1 === "Mr Robot")
        window.history.pushState({}, "", "/1player-3d");
    else
        window.history.pushState({}, "", "/2players-3d");
    handleLocation();
}
...
export function handleNextMatchClick3D() {
    const modal = document.getElementById('winMsgModal');
    modal.style.display = 'none';
    startNextMatch3D();
}

export async function showWinMessageTournament3D(winnerName) {
    const modal = document.getElementById('winMsgModal');
...
export async function showWinMessageEndTournament3D(championName) {
